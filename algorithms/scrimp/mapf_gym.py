...
from algorithms.scrimp.alg_parameters import EnvParameters, TieBreakingParameters, NetParameters
from algorithms.scrimp.od_mstar3 import od_mstar
from algorithms.scrimp.od_mstar3.col_set_addition import NoSolutionError
...
class State(object):
    ...
    def __init__(self, world0, goals, num_agents):
        ...
        self.agent_goals = goals.copy()
        self.num_agents = num_agents
        self.agents = self.scan_for_agents()  # position of agents
        self.heuri_map = np.zeros((self.num_agents, 4, *self.state.shape), dtype=bool)
        self.get_heuri_map()

        assert (len(self.agents) == num_agents)

    def update_goal(self, agent_idx, goal):
        self.agent_goals[agent_idx] = goal
        self.get_single_heuri_map(agent_idx)

    def scan_for_agents(self):
        """find the position of agents"""
    ...
    def imag_obs(self, agent_id, curr_position):
        ...
        for i in range(top_left[0], top_left[0] + EnvParameters.FOV_SIZE):  # top and bottom 
            for j in range(top_left[1], top_left[1] + EnvParameters.FOV_SIZE):  # left and right 
                ...
                if self.agent_goals[agent_id - 1] == (i, j):
    ...
    def value_compare(self, model, agent_indexes, pre_value, input_state, curr_position, past_position, valid_action,
        ...
        diff_dis = torch.nn.functional.softmax(torch.from_numpy(diffs), dim=-1)  # the final priority probability
    ...
    def joint_move(self, true_actions, model, pre_value, input_state, ps, no_reward, message,
        ...
        for i in range(self.num_agents):
            ...
            if collide_with_obstacle[i] != 1:
                ...
                if self.agent_goals[i] == (ax + dx, ay + dy):
                    agent_status[i] = 1  # reach goal
                    continue
                elif self.agent_goals[i] != (ax + dx, ay + dy) and self.agent_goals[i] == (ax, ay):
                    agent_status[i] = 2
    ...
    def task_done(self):
        ...
        for i in range(len(self.agents)):
            if self.agent_goals[i] == self.agents[i]:
                num_complete += 1
        if lifelong_mode:
            done = False
        else:
            done = num_complete == len(self.agents)
        return done, num_complete

    def get_single_heuri_map(self, agent_idx):
        dist_map = np.ones(self.state.shape, dtype=np.int32) * 2147483647
        open_list = list()
        x, y = tuple(self.agent_goals[agent_idx])
...
class MAPFEnv(gymnasium.Env):
    ...
    def set_state(self, grid, starts, goals):
        world = np.array(grid).astype(int)
        world *= (-1)
    ...
    def set_world(self):
        ...
        while goal_counter <= self.num_agents:
            ...
            if goals[x, y] == 0 and world[x, y] != -1:
                ...
                agent_goals.append((x, y))
                goal_counter += 1
        self.world = State(world, agent_goals, self.num_agents)
        ...
        for i in range(top_left[0], top_left[0] + self.observation_size):
            for j in range(top_left[1], top_left[1] + self.observation_size):  # left and right
                ...
                if self.world.state[i, j] == agent_id:
                    # own position
                    poss_map[i - top_left[0], j - top_left[1]] = 1
                if self.world.agent_goals[agent_id - 1] == (i, j):
                    # own goal
                    goal_map[i - top_left[0], j - top_left[1]] = 1
